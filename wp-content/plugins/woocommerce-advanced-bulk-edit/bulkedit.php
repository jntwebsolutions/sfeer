...
//				}
			...
			echo 'W3Ex._translate_strings["trans_use_builtin"] = "'.self::relpaceInvalid($arrTranslated['trans_use_builtin']).'";'; echo PHP_EOL;
		...
		<br/>
<!-- begin debug-->
		<!--<select id="testproductfind" class="makechosen" data-placeholder="choose" multiple style="width:250px;">
 ...
 <!-- end -->
            ...
            <template id="btn-info-filters-section-body">
                You can enable/disable filters from "Plugin Settings" > "Search Fields".
            </template>
			...
			<?php
				...
				if (file_exists( __DIR__.'/integrations/catnames-to-ids.php')) {
					foreach ($cats as $catInfo) {
						if (isset($catInfo->category_id) && isset($catInfo->category_name)) {
							$catInfo->category_name = $catInfo->category_id;
						}
					}
                }
				$newcats = array();
			    ...
			    foreach($cats as $cat)
				{
					...
					$cat->category_parent = (int)$cat->category_parent;
...
//								$woo_tags = get_terms( 'product_tag', $args );
			  ...
			  ?>/><?php _e( 'Variations', 'woocommerce');?></label>
			   ...
			   <span class="icon-floppy"></span>
				<?php _e("Save Changes","woocommerce-advbulkedit"); ?>
				 ...
				 <div style="display: inline-block;position: relative;width:320px;line-height:29px;">
					 <button id="btn-info-save-changes" class="btn-info"></button>
					 <template id="btn-info-save-changes-body">
						<?php //_e("Changes are saved on going to a different page of products, adding products/variations or via the 'Save Changes' button","woocommerce-advbulkedit"); ?>
						 Changes are saved on the following actions:
						 <ul>
							 <li>- Going to a different page of products</li>
							 <li>- Adding products/variations</li>
							 <li>- Clicking the 'Save Changes' button</li>
						 </ul>
					 </template>
				 </div>
			</div>
			...
			<?php
				if (file_exists( __DIR__.'/integrations/button-open-print-barcode-tabs.php')) {
					...
					W3ExABulkEdit_Integ_ButtonOpenPrintBarcodeTabs::addButtonOpenPrintBarcodeTabs();
					echo "<br/><br/>";
				}
			?>
			...
			<div style="position: relative;">
				...
				<button id="btn-info-linked-editing" class="btn-info"></button>
				<template id="btn-info-linked-editing-body">
					<?php _e( 'When enabled, any change you make on the grid to a single product, will apply to all selected products.<br/><br/>When disabled, the change will apply only for the edited product.', 'woocommerce-advbulkedit'); ?>
				</template>
			</div>
			<br />
			<div style="position: relative;" id="mainbuttons">
				...
				<input id="selectedit" class="button" type="button" value="<?php _e( "Selection Manager", "woocommerce-advbulkedit"); ?>" />
				...
				<div id="quicksettingsarea">
					<input id="quicksettingsbut" class="button" type="button" value="<?php _e( "Quick Settings", "woocommerce-advbulkedit"); ?>" />
				</div>
...
//						  	$settings = get_option('w3exabe_settings');
					...
					<tr>
						...
						<td width="50%"  style="padding-top: 25px;">
							...
							?>
							<option value ="1" <?php echo $normal; ?>><?php _e( 'normal', 'woocommerce-advbulkedit'); ?></option>
							<option value ="2" <?php echo $medium; ?>><?php _e( 'medium', 'woocommerce-advbulkedit'); ?></option>
							<option value ="3" <?php echo $big; ?>><?php _e( 'big', 'woocommerce-advbulkedit'); ?></option>
							</select>
						</td>
					</tr>
					<!--<tr>
					...
					<tr>
						...
						<td width="50%"  style="padding-top: 25px;">
							...
							?>
							>
							<?php _e( 'px', 'woocommerce-advbulkedit'); ?>
						</td>
					</tr>
<!--                    <tr>-->
<!--                        <td width="50%" style="padding-top: 20px;">-->
<!--                            <label><input id="usefixedjquery" type="checkbox"-->
<!--									--><?php
//										if(isset($settings['usefixedjquery']))
//										{
//											if($settings['usefixedjquery'] == 1)
//											{
//												echo 'checked=checked';
//											}
//										}
//									?>
<!--                                >--><?php //_e( 'Use fixed JQuery version (1.12.4)', 'woocommerce-advbulkedit'); ?><!--</label>-->
<!--                        </td>-->
<!--                        <td  style="padding-top: 20px;">-->
<!--                            /--><?php //_e( 'will need a page refresh', 'woocommerce-advbulkedit'); ?><!--/-->
<!--                        </td>-->
<!--                    </tr>-->
					<tr>
						<td width="50%" style="padding-top: 20px;">
			...
			<?php 
				...
				echo 'W3Ex.trans_movetrash = "'.__( "Move to Trash", "woocommerce-advbulkedit").'";'; echo PHP_EOL;
				echo 'W3Ex.trans_delperm = "'.__( "Delete Permanently", "woocommerce-advbulkedit").'";'; echo PHP_EOL;
...
//							$multiple = "multiple=''";
					?>
						 <select id="bulkgrouped_items" class="makechosen catselset" style="width:250px;" data-placeholder="select" <?php echo $multiple; ?>>
			...
			<div id="selquickactions">
				<?php _e( "Quick actions", "woocommerce-advbulkedit"); ?>:<br/>
				...
				(<?php _e( "same attributes", "woocommerce-advbulkedit"); ?> )<br/>
			...
			<table class="custstyle-table">
				<tr data-id="post_title" style="display: table-row;">
					...
					<td>
						...
						<select class="selectsplit" disabled="disabled"><option value="split"><?php _e('split commas', 'woocommerce-advbulkedit');?></option><option value="regexp"><?php _e('reg exp', 'woocommerce-advbulkedit');?></option></select>
						<select class="selectsplitand" disabled="disabled"><option value="and"><?php _e('AND', 'woocommerce-advbulkedit');?></option><option value="or"><?php _e('OR', 'woocommerce-advbulkedit');?></option></select>
					</td>
				</tr>
				<tr data-id="post_content">
					<td>
...
?></button>
			...
			<div id="categoriesdialog">
				...
				?>
					<ul class="categorychecklist form-no-clear <?php echo $classname; ?>">
					...
					<?php
						...
						if (file_exists( __DIR__.'/integrations/catnames-to-ids.php')) {
							...
							$args = array(
								...
								'popular_cats'          => false,
								'walker'                =>
									class_exists('W3ExABulkEdit_Walker_CategoryNamesToIDs_Checklist') ? new W3ExABulkEdit_Walker_CategoryNamesToIDs_Checklist() : null,
								'taxonomy'              => 'product_cat',
								'checked_ontop'         => true
							);
						}
